...
const AllActiveFuelRequestsAdmin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const completeRequest = (id: any) => {
        console.log("id");
      const  data={reqStatus:true}
        StationService.completeRequest(id,data).then(res=>{
                                                       ...
                                                       factoryList.filter((us) => us.status=="REQUESTED").      
                                                        map((us, index) => (
                                                            <tr key={index}>
                                                                ...
                                                                <td style={{ textAlign: "end", paddingRight: "55px" }}>
                                                                    <div className="d-flex p-0 float-right">

                                                                        <button type="button" className="btn updateBtn btn-sm mr-2" onClick={() => { completeRequest(us?._id); }}>Complete </button>
                                                                        <button type="button" className="btn inactiveBtn btn-sm mr-2" onClick={() => { completeRequest(us?._id); }}>Reject </button>
...
export default AllActiveFuelRequestsAdmin;
